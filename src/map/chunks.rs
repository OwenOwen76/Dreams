...
pub fn rotate_seed(time: Res<Time>, mut timer: ResMut<MapTimer>, mut seed: ResMut<MapSeed>) {
...
pub fn map_timer_finished(timer: Res<MapTimer>) -> bool {
    timer.0.just_finished()
}

pub fn tick_map_timer(time: Res<Time>, mut timer: ResMut<MapTimer>) {
    timer.0.tick(time.delta());
}
